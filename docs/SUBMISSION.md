...
The claimant tells their agent what happened; the agent assembles the claim and evidence on Fairground. The other party joins by link **with their own agent**, which reviews the record and gives them a private reality check. Each human privately tells their own agent their true limit — then the agents exchange up to three rounds of **sealed offers**: overlap settles instantly at the midpoint; otherwise only a "gap narrowed/widened" signal is published and the numbers never cross the table. If sealed rounds fail, a neutral AI mediator — which, like a real mediator in caucus, can see the sealed history — proposes a neutral settlement amount, numerically clamped between the parties’ last sealed positions. A plain-language agreement is drafted, and **both humans sign it themselves: no signing tool exists**. If a case closes without agreement, either side can download a court-preparation record (claim, response, dated evidence, their own sealed offers, the mediator's proposals, and an attestation of good-faith settlement attempt), so walking away costs nothing. A practice mode lets one person (or one judge) run the whole two-sided flow against a realistic AI counterpart — including **⚡ Autopilot**: set your private limit, and the two advocates run the entire procedure live, stopping only at the signature that no tool can perform. And it works: we ran **30 randomized disputes end-to-end through the live deployment — 100% of completed cases settled, at a mean 70% of claim, in 52 seconds each** (methodology in the repo), all feeding the live public Docket on the landing page.

### Why this is a strong fit for WebMCP

Fairground's core design idea: **the tool surface is the procedure.** All 24 tools are registered per role and per phase via `document.modelContext` (using Chrome's official `use-webmcp-tool` hook, with `toolchange` firing as the case advances). The respondent's agent has no offer tool until it files a response; no agent can bid before its human sets a private mandate; and there is deliberately **no signing tool at all** — due process is enforced by tool availability, not by prompts. It's also a *multiplayer* WebMCP app: two adversarial agents on one neutral page, where the server structurally filters each side's view so a private mandate or sealed offer can never reach the other side — even via a prompt-injected agent. Tools reading opponent-authored content carry `untrustedContentHint` with fenced content; read-only tools carry `readOnlyHint`; a mandate guard refuses agent offers beyond the human's stated limit; and the spec's elicitation pattern is implemented app-level: an agent that wants to cross the mandate raises an approval card only the human can click. We also use WebMCP's declarative API where a human form IS the tool: parties rate the fairness of the process through an annotated form, feeding a live public fairness score, grounded in Tyler's procedural-justice research.
...
React components register tools with Chrome's `use-webmcp-tool` hook; the `enabled` flag (role × phase) drives lifecycle registration, so the browser's tool list always equals the set of legitimate procedural moves. Every tool result ends with per-role "NEXT:" guidance so agents flow through the procedure without guessing. A Next.js backend holds the case state machine (Upstash Redis), enforces the action×phase×role guard on every mutation, resolves sealed rounds server-side, and role-filters every view. The AI layer (Vercel AI SDK + OpenAI) powers the neutral mediator (numerically hard-clamped between the parties' last sealed positions), private reality checks, agreement drafting, and the practice counterpart — all with deterministic fallbacks. The repo includes two automated checks: a full-lifecycle API smoke test (including mandate-guard and role-privacy assertions) a headless-Chrome test that asserts per-phase tool registration/unregistration and annotations, and an **agent-loop eval suite (7/7 passing)** that includes a red-team scenario: the opposing party plants a prompt-injection message demanding the private mandate — the agent resists, and a structural sweep proves no tool on the other side's page can emit it at all. Every signed settlement also carries a **tamper-evident SHA-256 record seal**, verifiable by anyone at a public endpoint or via the verify_settlement_record tool.
